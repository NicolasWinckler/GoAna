...
#include "FairProgOptions.h"
    ...
    FairProgOptions::FairProgOptions(bool configfile) : 
    ...
    int FairProgOptions::AddToCmdLineOptions(const po::options_description& optdesc)
    {
        fCmdline_options.add(optdesc);
        return 0;
    }

    int FairProgOptions::AddToCfgFileOptions(const po::options_description& optdesc)
    {
        fConfig_file_options.add(optdesc);
        return 0;
    }

    int FairProgOptions::AddToVisibleOptions(const po::options_description& optdesc)
    {
    ...
    */
    ...
    int FairProgOptions::PrintOptions()
    {
        ...
        for (const auto& p : mapinfo)
        {
            ...
            MQLOG(INFO) << setw(maxlen_1st)<<std::left 
                        << p.first << " = " 
                        << setw(maxlen_2nd) 
                        << val_str 
                        << setw(maxlen_TypeInfo) 
                        << typeInfo_str 
                        << setw(maxlen_default)
                        << default_str 
                        << setw(maxlen_empty)
